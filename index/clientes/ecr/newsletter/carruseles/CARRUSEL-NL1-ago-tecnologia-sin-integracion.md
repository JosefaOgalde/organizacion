# ECR · Carrusel LinkedIn — NL 1 ago
**Artículo:** Tecnología sin integración: el principal freno de la eficiencia  
**Tarea:** `[ECR] TI — Carrusel` · #07 · 2026-07-21  
**Formato Canva:** 1080×1080 (carrusel LinkedIn)  
**Marca:** ECR GROUP® · naranja #E85D04 / tipografía limpia · logo en portada y cierre

Copy del post (pegar al publicar): ver `../copys/COPY-tecnologia-sin-integracion.txt` → sección **2) CARRUSEL — COPY** (elegir A o B).

---

## Slides (texto listo para Canva)

### Slide 1 — Portada
**Título:** Tecnología sin integración  
**Subtítulo:** El principal freno de la eficiencia  
**Pie:** ¿Más tecnología… o mejores resultados?

### Slide 2 — El problema
**Texto (2–3 líneas, sin viñetas):**  
Digitalizaron el reporte, no la decisión.  
Más plataformas no siempre significa mejor operación.  
El dato existe… pero no cambia el resultado.

### Slide 3 — La desconexión
**Título:** El problema no es la herramienta  
**Cuerpo:** Es la desconexión.  
- Dato que llega tarde  
- Alertas sin dueño  
- Equipos que siguen en WhatsApp

### Slide 4 — Retail / terreno
**Título:** En el punto de venta se nota  
**Cuerpo:** Campaña impecable en PowerPoint…  
pero si el equipo no llega, la reposición falla o la alerta no se gestiona a tiempo, el impacto se diluye.

### Slide 5 — Causa real
**Título:** No es falta de digitalización  
**Cuerpo:** Es falta de integración.  
Sistemas que no conversan con la operación real: turnos, supervisión, reposición, asistencia.

### Slide 6 — Qué hacen los líderes
**Título:** Las empresas más eficientes…  
**Cuerpo:** No suman más plataformas.  
Conectan personas, procesos y tecnología para decidir a tiempo.

### Slide 7 — Cadena completa
**Título:** Eficiencia = cadena completa  
**Cuerpo:**  
1. Información confiable  
2. Procesos definidos  
3. Equipos preparados  
4. Capacidad de respuesta

### Slide 8 — Cierre + CTA
**Título:** Integrar para decidir mejor  
**Cuerpo:** Cuando los sistemas conversan con la operación, la eficiencia deja de ser promesa y se vuelve resultado.  
**CTA:** Lee el artículo completo → (pegar link)  
**Marca:** ECR GROUP®

---

## Checklist Canva
- [ ] 8 slides 1080×1080  
- [ ] Logo ECR en slide 1 y 8  
- [ ] Contraste legible (texto oscuro / fondo limpio o portada Midjourney suave)  
- [ ] Misma tipografía en toda la serie  
- [ ] Export PNG/JPG secuencia  
- [ ] Publicar con copy Carrusel A o B + link al artículo  

## Relacionados
- Artículo: `../articulos/ART-tecnologia-sin-integracion.txt`  
- Copys: `../copys/COPY-tecnologia-sin-integracion.txt`  
- Portadas: `../portadas-guardadas/NL1-ago-portadas-canva-finales.md`

## Fondos Midjourney (cuadrados)
Ver: [PROMPTS-FONDOS-CUADRADOS-NL1-ago.md](./PROMPTS-FONDOS-CUADRADOS-NL1-ago.md)

