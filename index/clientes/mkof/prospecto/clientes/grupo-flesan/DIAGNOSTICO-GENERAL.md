# Grupo Flesan — Diagnóstico general (prospecto MKOF)

**Nivel:** general (no tan detallado como Clínica Indisa)  
**Fecha consulta web:** 2026-07-27  
**Objetivo MKOF:** entender el escenario de marca / webs / RRSS para ofrecer servicios.

---

## 1. Contexto y desafío de marca

Hoy conviven **tres nombres** según el negocio:

| Nombre | Uso aparente |
|--------|----------------|
| **Grupo Flesan** | Paraguas corporativo / holding |
| **Flesan** | Unidades especializadas (infra, demoliciones, geotecnia, minería…) |
| **DVC** (De Vicente Constructora) | Edificación / constructora |

**Problema declarado:** definir **cuál es el nombre adecuado** y armar una **estrategia de marca** coherente.

Lectura general: no es solo un tema de logo. Es un problema de **arquitectura de marca** (holding vs marcas operativas) que hoy se refleja en **tres sitios web** y un solo set de RRSS.

---

## 2. Ecosistema web (revisión general)

| Sitio | Rol | Lectura rápida |
|-------|-----|----------------|
| [grupoflesan.com](https://www.grupoflesan.com/) | Casa matriz del grupo | Presenta el ecosistema: Ingeniería & Construcción, Servicios Integrales, Inversiones. Empresas: DVC, Flesan Infraestructura, Geotecnia, Demoliciones, FAI, DIFAI. Mensaje de sinergia Chile–Perú (+43 años). |
| [flesan.cl](https://flesan.cl/) | Marca Flesan / unidades | Enfasis en “Somos Flesan” + verticales (infra, demoliciones, geotecnia, minería). Noticias de obras y alianzas. |
| [dvc.cl](https://www.dvc.cl/) | De Vicente Constructora | Edificación integral (retail, habitacional, institucional, etc.). Se presenta **como parte de Grupo Flesan**. |

**Hallazgo clave (general):**  
Los tres sitios **comparten los mismos links de RRSS en el footer** → la conversación digital pública se concentra en **Grupo Flesan**, no en DVC/Flesan por separado. Eso refuerza la tensión: webs fragmentadas por marca operativa + voz única en redes.

**Implicancia para oferta MKOF:**  
Prioridad alta en **estrategia de marca / naming / arquitectura** + alineación de sitios y tono, antes (o junto a) tácticas de contenido.

---

## 3. RRSS — panorama general

Fuente de perfiles (footer de los 3 sitios):

| Red | URL | Números (2026-07-27) | Método |
|-----|-----|----------------------|--------|
| Instagram | https://www.instagram.com/grupoflesan/ | **4.168** seguidores · 466 posts · ER muestra **1,82%** (N=5) | Screenshot 2026-07-28 |
| LinkedIn | Grupo Flesan Chile | **33 mil** seguidores · 1 mil–5 mil emp. · Las Condes | Screenshot 2026-07-28 |
| Facebook | https://www.facebook.com/grupoflesanchile | **1,2 mil** seguidores · 12 seguidos | Screenshot 2026-07-28 |
| YouTube | https://www.youtube.com/@GrupoFlesan | **Pendiente** — captura usuaria | URL footer sitios |

### Tipo de comunicación (lectura general, sin ER por post)

- Marca en redes = **Grupo Flesan** (no perfiles separados DVC/Flesan visibles en footer).
- Rubro B2B construcción / ingeniería: suele priorizar obras, hitos, seguridad, comunidad y empleador; conviene validar con el detalle de IG.
- YouTube: canal existe (@GrupoFlesan); números pendientes de captura → validar audiencia antes de proponer video.

> Instagram cargado: `@grupoflesan` · collabs frecuentes con DVC (`@devicenteconstructora`) y Flesan IC (`@flesan.ic`). Muestra ER con posts más antiguos que la de competidores.

---

## 4. Competencia (directa, según cliente)

| Competidor | Web (referencia) | Nota |
|------------|------------------|------|
| **Echeverría Izquierdo** | https://ei.cl/ | Referente · IG 8.649 · LI **129 mil** |
| **Inarco** (Constructora INARCO S.A) | https://inarco.cl/ | Comp.2 · IG **1.517** (@inarcochile) · ER **4,17%** · LI **66 mil** · FB **no tiene** |
| **Icafal** (Icafal S.A.) | https://www.icafal.cl/ | Comp.3 · IG **4.782** (@icafal_sa) · ER **1,04%** · LI **65 mil** |

> Alcance matriz: **solo estos 3 competidores** (+ Prospecto).

### Referente: Echeverría Izquierdo — Competidor 1 (matriz)

Revisión general de [ei.cl](https://ei.cl/) + capturas usuaria **2026-07-28**:

| Red | Dato | Fuente |
|-----|------|--------|
| Instagram | **8.649** seguidores · 1.076 posts · 351 seguidos · `@echeverria.izquierdo` | Perfil IG (ima 3) |
| IG ER (muestra) | **0,58%** · N=5 últimos posts · Σ likes 251 · comentarios visibles 0 | Imas 4–8 → `RRSS/data/ig-er-ei-muestra.json` |
| LinkedIn | **129 mil** seguidores · +10 mil empleados · Construcción · Santiago | Página empresa (ima 9) |
| Facebook / YouTube / TikTok | Pendiente | — |

Temáticas IG observadas (últimos 5): innovación/transformación digital, LPS 360 en obra, liderazgo (UC), sostenibilidad (Best Performer), networking corporativo. Formatos: video + carrusel con overlay de marca.

- Marca clara bajo **“Ei / Echeverría Izquierdo”** (un nombre dominante en home).
- Home con **propuesta + datos de negocio** → tono corporativo / inversor + industria.
- Estructura por empresas del grupo, pero **paraguas verbal único**.

**Contraste general con Flesan:**  
Echeverría Izquierdo proyecta **una marca holding fuerte** + IG corporativo activo + LI masivo (129 mil). Flesan/DVC/Grupo Flesan proyectan **varias puertas de entrada** (3 webs) con **una sola voz en RRSS**. El gap de estrategia de marca es evidente.

---

## 5. Materiales internos (cliente)

Ruta en PC (pendiente copiar al repo):

- Afiche Obra Nuevos Valores.pdf  
- Tarjetas nuevos valores Anverso & Reverso.pdf  
- Comunicación y Marketing — Manual de Marca 2026.pdf  

Útiles para cruzar **valores / tono / usos de marca** vs lo que se ve online. En cloud aún no están.

---

## 6. Oportunidades en RRSS (diagnóstico)

Fuente: `data/oportunidades.json` · `RRSS/laminas/TEXTOS-OPORTUNIDADES.md`

1. **Fragmentación de marca en redes** — En Instagram la cuenta es una, pero en los posts aparecen varias marcas a la vez. Eso diluye quién habla.
2. **Temática muy acotada a obra** — Casi todo el contenido es proyecto e hito B2B. Poco se ve de personas, cultura o empleador frente a la competencia.
3. **Ritmo bajo e irregular + rezago en LinkedIn** — Publican poco y sin constancia (0 a 1 por semana), mientras la competencia va 2–4 veces por semana. En LinkedIn quedan atrás en alcance (33 mil vs aproximadamente 65–129 mil).

---

## 7. Conclusiones (una página)

- El dolor central no es “falta de posts”: es **identidad de marca fragmentada**.
- Tres webs + un pack de RRSS = síntoma de **holding multi-empresa sin jerarquía comunicacional clara**.
- Competencia directa citada; **Echeverría Izquierdo** es el espejo de “cómo se ve una marca clara en web/comms”.
- MKOF puede entrar por **estrategia de marca + sistema de comunicación** (web + RRSS), no solo por pauta orgánica.

**Siguiente paso:** FB Icafal / YT-TikTok pendientes → cerrar matriz.
