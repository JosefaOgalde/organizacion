(function () {
  ...
  function render() {
    ...
    let editPath = '';
    if (item.editar) {
      if (item.editar.startsWith('index/') || item.editar.startsWith('data/') || item.editar.startsWith('docs/')) {
        editPath = item.editar;
      } else if (/\.(md|html)$/i.test(item.editar)) {
        editPath = `index/clientes/mkof/${item.editar}`;
      } else {
        editPath = `data/${item.editar}`;
      }
    }
    void baseData;
