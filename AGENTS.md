...
Resumen 1 línea: panel `…/impresoreando/panel/` · live `data/impresoreando-live.json` · API `/api/impresoreando` · pedidos `PED-00n` · ventas `I00000n` · historial · solo ventas bajan deuda · **«pagado» = venta** (transferir al tiro) · PLA+ negro `$17986/kg`. **Impresoras:** «nueva»/Elegoo = Centauri Carbon 2 · «antigua» = Ender 3 V2 Neo Sprite Neo (otro filamento).
...
**Frases gatillo:** «pedidos e impresoreando» → pedir nombre+origen+ítems+precio; si dice **pagado** → PED + transferir a `I00000n`. «calcular costo producto impresoreando» → pedir imagen. Estado de pedidos visible en Resumen.
