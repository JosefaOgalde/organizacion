...
| Sync → organizador | `node scripts/sync-impresoreando-pedidos-organizacion.js --also-respaldo` (también `ABRIR-LARAVEL.bat`). **1 subtarea por PED activo** (`[IMP] PED-00n · …`); **transferidos/cerrados se eliminan** de subtareas (no quedan tachados). **Fiados** → tarea `[IMP] Cobrar PED-00n · …` el día `fechaPagoEsperada`. **Madre siempre en fecha de hoy (Chile)**. |
